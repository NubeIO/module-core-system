...
// TODO: Known Issue
//  1) If exception event has passed, it will not be included in the exception ScheduleCheckerResults, and therefore
//  if a schedule period start was before the exception period, the final result will not show the correct PeriodStart

// ExceptionCheck checks all Exception Schedules in the payload for active periods.
// It returns a combined ScheduleCheckerResult of all Exception Schedules.
func ExceptionCheck(exceptions TypeEvents, scheduleName, timezone string) (ScheduleCheckerResult, error) {
	...
	// Once the ScheduleCheckerResult has been computed (as an Event type schedule) then set the IsException flag.
	// This result should later be combined with Event and Weekly schedules.
	results.IsException = true
	return results, nil
}

// ApplyExceptionSchedule Combines ScheduleCheckerResults with an Exception ScheduleCheckerResults.
// The IsException flag must be set for the Exception ScheduleCheckerResult.
